function [ydata, kld] = tsne_PCA(X, labels, no_dims, initial_dims, perplexity)
...
%
% MODIFIED BY CVI MARCH 2012 - formatted to take data pre-processed
% using PCA (in higher dimensions than the final output!)
% in order to facilitate the calculaition of a transform in getSSIMS.m
...
%     end
    ...
    % Run t-SNE
	if exist('tsne_p_PCA_helper', 'file') == 3
		if initial_solution
			[ydata, kld] = tsne_p_PCA_withMEX(P, labels, ydata);
		else
			[ydata, kld] = tsne_p_PCA_withMEX(P, labels, no_dims);
		end
	else
		if initial_solution
			[ydata, kld] = tsne_p_PCA(P, labels, ydata);
		else
			[ydata, kld] = tsne_p_PCA(P, labels, no_dims);
		end
	end
