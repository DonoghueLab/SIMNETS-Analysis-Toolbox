function [ydata, cost] = tsne_p_PCA(P, labels, no_dims)
...
% University of California, San Diego
    ...
    if ~initial_solution
        ...
        %%% added by CVI
        prinC = getPC(P);
        ydata = getPCP(P,prinC,no_dims)';
        %ydata =  (ydata/max(max(ydata)));
        for nd = 1:no_dims
            ydata(:,nd) =  ydata(:,nd) / max(ydata(:,nd));
        end
        %%%
        %%%%%%%%%%%%%%%%%%%%%
    end
    y_incs  = zeros(size(ydata));
    gains = zeros(size(ydata)) + 1;
    ...
    for iter=1:max_iter
        ...
        if ~rem(iter, 10) && ~isempty(labels)
            ...
            elseif no_dims == 2
                h = scatter(ydata(:,1), ydata(:,2), 40, labels, 'filled');
                 set(h,'linewidth',3)
            else

                h = scatter3(ydata(:,1), ydata(:,2), ydata(:,3), 40, labels, 'filled');
               set(h,'linewidth',3)
               % colstr = 'rbgkmc'
              %  scatter3(ydata(:,1), ydata(:,2), ydata(:,3), 40, colorstr(1:length(labels)), 'filled');
            end
            axis tight
            axis off
            drawnow
